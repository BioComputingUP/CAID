...
def parse_reference(ref_file: str, pttrn: dict=None, accs_to_read: set=None) -> (dict, set):
    ...
    with open(ref_file) as f:
        ...
        for header in faiter:
            ...
            if header[0] != '#':
                ...
                if (accs_to_read is not None and acc in accs_to_read ) or accs_to_read is None:
...
def parse_prediction(predfile, reference_ids, label=None, decimals=3, threshold=0.5, normalize=False):
    ...
    :rtype:
    """
    logging.debug('Parsing prediction file: {}'.format(predfile))
    ...
    with open(predfile.resolve()) as fhandle:
        ...
        for acc in faiter:
            ...
            if acc in reference_ids:
                splitted = list(zip(*map(strip_split, body)))
                scores = None
                states = None

                if len(splitted) == 3:
                    positions, _, scores = splitted
                if len(splitted) == 4:
                    positions, _, scores, states = splitted
                ...
                if states is not None and states[0]:
