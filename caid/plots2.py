...
from scipy.stats import hmean, pearsonr, ttest_ind
...
def plot_metric_to_threshold(metric, dataset_perthr_metric, default_thr, opt_threshold, outdir, outbase, names=None):
    ...
    for i, (ax, p) in enumerate(zip(axes.ravel(), sorted(dataset_perthr_metric, key=df_sorter))):
        ...
        ax.axvline(opt_threshold[method], linestyle="--", label="Optimal: {}".format(opt_threshold[method]),
                   color="deeppink")
    ...
    fig.tight_layout()
    plt.savefig(outdir / "{}{}ToThr.png".format(outbase + "_" if outbase else outbase, metric), dpi=dpi,
                bbox_inches="tight")
    plt.close()
...
def plot_pertarget_permethod_heatmap(metric, target_metrics_preds, target_metrics_bases, outdir, outbase, names=None):
    ...
    ax = sns.heatmap(tgt_metric.reindex(tgt_pred_metrics_avg_col.index, axis=1).reindex(tgt_pred_metrics_avg_row.index),
                     ax=ax)
    ax2 = ax.twiny()
    ax2.set_xticks(ax.xaxis.get_ticklocs())
    ax2.set_xticklabels(
            tgt_pred_metrics_avg_col.values[[list(tgt_pred_metrics_avg_col.index).index(l.get_text()) for l in
                                             ax.xaxis.get_ticklabels()]].round(3))
    ax2.tick_params(axis='x', rotation=90)

    plt.savefig(outdir / "{}tgtheatmap_{}.png".format(outbase + "_" if outbase else outbase, metric), dpi=dpi,
                bbox_inches="tight")
    plt.close(fig)
...
def plot_icontent_correlation(metric, predictions, cons, pdbr, gene3dr, outdir, outbase, names=None):
    ...
    predictions = pd.DataFrame(
            {**predictions.to_dict(), **cons.to_dict(), **pdbr.to_dict(), **gene3dr.to_dict()}).dropna()
    ...
    plt.savefig(outdir / "{}icontentcorr_{}.png".format(outbase + "_" if outbase else outbase, metric), dpi=dpi,
                bbox_inches="tight")
    plt.close()
...
def plot_average_overall_ranking(metric, metrics_preds, metrics_bases, outdir, outbase, plotfirst=None, names=None,
                                 level='target'):
...
def plot_dat_tgt_metric_cpuspeed(metric, dat_metr_preds, dat_metr_bases, tgt_metr_preds, tgt_metr_bases, bts_ci_preds,
                                 outdir, outbase="", names=None):
    ...
    colors = ['silver'] * len(tgt_metr_preds.groupby(level=0).mean()) + ['grey'] * len(
    ...
    plt.savefig(outdir / "{}bar_{}.png".format(outbase + "_" if outbase else outbase, metric), dpi=dpi,
                bbox_inches="tight")
    plt.close(fig)


def plot_dataset_target_metric(metric, dat_metr_preds, dat_metr_bases, tgt_metr_preds, tgt_metr_bases, bts_ci_preds,
                               outdir, outbase="", names=None):
    logging.info("Plotting metric barplot: {}, {}".format(metric, outbase))
    fig, axes = plt.subplots(1, 2, figsize=(15, 6), sharey=True)

    negatives, optm = outbase.split("_opt")
    # optm = optm[3:]

    # print('metric', metric)
    # print('dat_metr_preds', dat_metr_preds.columns)
    # plot dataset metrics on left subplot
    dat_m = dat_metr_preds[metric].sort_values(ascending=False).append(
            dat_metr_bases[metric].sort_values(ascending=False))
    # yerr = bts_ci_preds.xs(metric, level=1)[["lo", 'hi']].reindex(dat_m.index)
    ...
    ax = dat_m.plot.bar(ax=axes[0], color=colors)
    ...
    # plot target metrics on right subplot
    tgt_bases_grouped = tgt_metr_bases.groupby(level=0)
    tgt_preds_grouped = tgt_metr_preds.groupby(level=0)

    n = tgt_preds_grouped.count().append(tgt_bases_grouped.count())
    tgt_m = tgt_preds_grouped.mean()[metric].sort_values(ascending=False).append(
            tgt_bases_grouped.mean()[metric].sort_values(ascending=False))
    yerr = tgt_preds_grouped.std()[metric].append(tgt_bases_grouped.std()[metric])[tgt_m.index] / n[metric] ** 0.5 / 2
    colors = ['silver'] * len(tgt_preds_grouped.mean()) + ['grey'] * len(tgt_bases_grouped.mean())

    if names is not None:
        tgt_m = tgt_m.rename(names)
        yerr = yerr.rename(names)

    ax = tgt_m.plot.bar(ax=axes[1], color=colors)
...
def plot_roc(preds_rocs, cons_roc, pdb_roc, gene3d_roc, random_rocs, outdir, outbase, names=None):
    ...
    #     markerfacecolor='w', markersize=10, c='w', label=names(n.columns.get_level_values(0)[0]))
...
def plot_pr(pred_prs, cons_prc, pdb_prc, gene3d_prc, random_prcs, cov, outdir, outbase, sortby="auc", names=None):
    ...
    fmax_pr = sorted(((p, hmean(pred_prs[p].dropna().values, axis=1).max().round(2)) for p in
    ...
    ax.legend(lhandles,
              ['{}'.format('{} - {}: {}, C: {:.2f}'.format(names(l), sortby.upper(), sorter[pwauc.index(l)][1],
                                                           cov[l]) if l in pwauc else l) for l in llabels],
    ...
    fig.tight_layout()
    plt.savefig(outdir / "{}pr.{}.png".format(outbase + "_" if outbase else outbase, sortby), dpi=dpi,
                bbox_inches="tight")
    plt.close(fig)
...
def plot_cputime_to_performance(metric, tgt_pred_metrics, outdir, outbase, names=None):
    ...
    cputime = pd.read_csv('../data/dataset_stats/cpu_time.csv', header=[0, 1], index_col=[0]).groupby(level=0,
                                                                                                      axis=1).sum()
...
def make_plots(reference):
    ...
    roc_random_bases = [pd.read_csv(baseln_dir / '{}.{}.all.target.mcc.metrics.csv'.format(refname, b), index_col=0) for
                        b in basetypes]

    pr_preds_f = result_dir / "{}.analysis.all.dataset._.pr.csv".format(refname)
    ...
    #                        header=[0, 1, 2, 3])
    pr_random_bases = [pd.read_csv(baseln_dir / '{}.{}.all.target.mcc.metrics.csv'.format(refname, b), index_col=0)
                       for b in basetypes]

    plot_roc(roc_preds, None, None, None, roc_random_bases, output_dir, refname, names=get_names)
    coverage = pd.read_csv(result_dir / '{}.analysis.all.target.default.metrics.csv'.format(refname), index_col=[0, 1],
                           header=[0, 1])
    ...
    codestart = 'D' if 'disorder' in str(ref_dir) else 'B'
    dataset_metrics_single_preds = [pd.read_csv(f, index_col=[0, 1]) for f in
                                    result_dir.glob("{}.analysis.{}*dataset*".format(Path(refname).stem, codestart))]
    ...
    for optimized_metric in ["default", "f1s", "mcc", "bac"]:
        ...
        target_metrics_preds_f = result_dir / "{}.analysis.all.target.{}.metrics.csv".format(refname,
                                                                                             optimized_metric)
        ...
        for bt in basetypes:
            ...
            if bt in basetypes:
                target_metrics_base_f = baseln_dir / "{}.{}.avg.target.{}.metrics.csv".format(refname, bt,
                                                                                              optimized_metric)
                target_metrics_bases.append(
                        pd.read_csv(target_metrics_base_f, index_col=[0, 1], header=[0, 1]).xs('mean', level=1,
                                                                                               axis=1))
                dataset_metrics_base_f = baseln_dir / "{}.{}.avg.dataset.{}.metrics.csv".format(refname, bt,
                                                                                                optimized_metric)
                dataset_metrics_bases.append(
                        pd.read_csv(dataset_metrics_base_f, index_col=0).loc["mean"].to_frame(bt).T)
            else:
                target_metrics_base_f = baseln_dir / "{}.{}.all.target.{}.metrics.csv".format(refname, bt,
                                                                                              optimized_metric)
                target_metrics_bases.append(pd.read_csv(target_metrics_base_f, index_col=[0, 1]))
                dataset_metrics_base_f = baseln_dir / "{}.{}.all.dataset.{}.metrics.csv".format(refname, bt,
                                                                                                optimized_metric)
        ...
        for metric_to_plot in ["f1s"]:
            if metric_to_plot not in {"aucroc", "aucpr", "thr", "aps"}:
                plot_dataset_target_metric(metric_to_plot, dataset_metrics_preds, dataset_metrics_bases,
                                           target_metrics_preds, target_metrics_bases, None, output_dir,
                ...
                plot_pertarget_permethod_heatmap(metric_to_plot, target_metrics_preds, target_metrics_bases,
                                                 output_dir, "{}_opt{}".format(refname, optimized_metric),
                                                 names=get_names)
                # plot_methdod_correlation(metric_to_plot, target_metrics_preds, outputdir,
                #                          "{}_opt{}".format(refname, optimized_metric))

        # plot_average_overall_ranking(optimized_metric, target_metrics_preds, target_metrics_bases, outputdir,
        #                              refname, names=get_names, level='target')
        ...
        # plot_average_overall_ranking(optimized_metric, target_metrics_preds, target_metrics_bases, outputdir,
        #                              refname, plotfirst=10, names=get_names, level='target')
        # plot_average_overall_ranking(optimized_metric, dataset_metrics_preds, dataset_metrics_bases, outputdir,
        #                              refname, plotfirst=10, names=get_names, level='dataset')
...
excluded_refs = ["pdb-atleast", "pdb-nucleic_acid_binding-atleast"]
...
if __name__ == "__main__":
    ...
    baseln_dir = Path(args.baselineDir)
    output_dir = Path(args.outputDir)

    os.makedirs(output_dir, exist_ok=True)

    ref_dir = Path(args.referenceDir)
    references = list(filter(lambda x: x.stem not in excluded_refs, ref_dir.glob(args.glob)))
    ...
    basetypes = ["random", "shuffledataset"]  # random baselines
    ...
    with mp.Pool(processes=mp.cpu_count() - 1) as p:
        list(tqdm(p.imap(make_plots, references), total=len(references), desc="Plotting results for each reference"))
